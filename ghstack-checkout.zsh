...
_ghstack_checkout_pick() {
  if [[ $LBUFFER == *ghstack\ checkout* ]]; then
    ...
    zle "$_ghstack_checkout_fallback"
  fi
}

# Remember whatever TAB was bound to before we take it over, so normal
# completion still works everywhere else.
_ghstack_checkout_fallback=${$(bindkey '^I')[(w)2]}
[[ $_ghstack_checkout_fallback == _ghstack_checkout_pick || -z $_ghstack_checkout_fallback ]] \
  && _ghstack_checkout_fallback=expand-or-complete

zle -N _ghstack_checkout_pick
bindkey '^I' _ghstack_checkout_pick
